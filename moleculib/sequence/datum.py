...
class SeqDatum:
    ...
    def empty_seqrecord(cls):
        return cls(idcode="", sequence=ProteinSequence(""), residue_token=None)
    ...
    def from_filepath(cls, filepath):
        ...
        residues = [
            (
                "UNK"
                if (ProteinSequence.convert_letter_1to3(name) not in all_residues)
                else ProteinSequence.convert_letter_1to3(name)
            )
            for name in list(selected_sequence)
        ]

        residue_tokens = np.array(
            list(map(lambda res: get_residue_index(res), residues))
        )
