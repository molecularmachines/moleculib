...
def pdb_to_atom_array(pdb_path, cif, model=None, chain=None, RNA=False, id=None):
    ...
    if RNA == True:
        ...
        RNA_filter = np.logical_and(
            no_dna_filter, nuc_filter
        )  # no DNA and only nucleotides filter
...
class NucleicDatum:
    ...
    def _extract_reshaped_atom_attr(
        ...
        for attr in attrs:
            ...
            if len(attr_shape) == 1:
                ...
                attr_reshape = np.zeros(
                    (residue_count, MAX_DNA_ATOMS, attr_shape[-1])
                )  # NOTE: do we want to change it from 506,3 to 16, 24, 3?? its much less than 506... 1518 -->1152
        ...
        def _atom_slice(atom_name, atom_array, atom_token):
            ...
            atom_array_ = atom_array_[
                (atom_array_.residue_token != 12)
            ]  # UNK is 13 #NOTE: WE BASICALLY CANCEL UNK?

            if atom_name not in backbone_atoms_RNA:
                atom_array_ = atom_array_[
                    (atom_array_.residue_token != 13)
                ]  # >1 #PAD IS 12 ##flag as masks
    ...
    def from_filepath(
        ...
        atom_array = pdb_to_atom_array(
            filepath, cif, model=model, chain=chain, RNA=True
        )  # NOTE: CHANGE RNA TO TRUE IF WANT ONLY RNA. filters pdb to only nucleotides
    ...
    def from_atom_array(
        ...
        res_names = [("UNK" if (name not in all_nucs) else name) for name in res_names]
        ...
        residue_mask = np.ones_like(residue_token).astype(
            bool
        )  # creates array of same shape as res token, with all True
        ...
        chain_res_cumsum = np.cumsum(
            [0] + list(chain_res_sizes[:-1])
        )  # getting rid of last element, starting from 0.
    ...
    def to_pdb_str(self):
        ...
        all_atom_res_indices = repeat(np.arange(len(self.nuc_token)), "r -> r a", a=24)[
            atom_mask
        ]
    ...
    def plot(
        ...
        if ribbon:
            view.addStyle(
                {"model": -1}, {"cartoon": {"color": color}}, viewer=viewer
            )  # may need to change to stick if doesnt work
